<template>
  <div>
    ...
    <div v-if="userType == 'client'">
      ...
      <p v-else="!myVideos.length">
        No videos
      </p>

      <h4 class="sub-heading">
        Your Virtual Assistant(s) Videos
      </h4>
      <div
      ...
      >
        <video-thumbnail
          v-for="video in associateVideos"
          :key="video.id"
          :video="video"
        />
      </div>
      <p v-else="!associateVideos.length">
        No Videos
      </p>
    </div>
    <div v-else>
    ...
    >
      <v-card class="video-player-container p-2 d-flex flex-column">
        <v-card-title class="d-flex flex-row">
          ...
          <v-btn
            class="ml-auto my-hover-btn"
            icon
            @click="setClickedVidUrl('')"
          >
            <v-icon>{{ icons.mdiClose }}</v-icon>
          </v-btn>
        </v-card-title>
        <video
          class="m-2 video-player"
          :src="state.clickedVideoUrl"
          controls
        />
      </v-card>
    </v-dialog>
...
export default {
  ...
  setup() {
    const {
      state, setClickedVidUrl, activeVideo, myVideos, associateVideos,
    } = useVideos()
